
# Hi there, I'm Osama Khan 👋

### 👨‍💻 About Me
- 🎓 **Education:** Intermediate in Computer Science (ICS) | School Topper
- 🏫 **Experience:** Teacher at Iqra Public School (Teaching CS, Science & English)
- 🎯 **Current Focus:** Preparing for GKS Scholarship & Pursuing BS in Computer Science & AI in South Korea
- 💡 **Interests:** Artificial Intelligence, Machine Learning, Web Development, and Language Learning

---

### 🛠️ Languages & Tools
- **Languages:** Python, C++, HTML5, CSS3, JavaScript
- **Tools & Platforms:** VS Code, Cursor, Git & GitHub

---

### 🚀 Projects
- 📌 **Student Result Predictor:** Python script evaluating student academic metrics to predict performance outcomes.
- 📌 **Spam Email Detector:** Rule-based NLP text classification algorithm for email filtering.

---

### 🌐 Connect & Languages
- 🗣️ **Languages Spoken:** English, Urdu, Pashto, Beginner Korean (TOPIK Prep)
- 📌 **GitHub:** https://github.com/osamakhan-dev
<!--
**osamakhan-dev/osamakhan-dev** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
